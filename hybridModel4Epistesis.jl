...
function bayesPR_selReg(genoTrain, phenoTrain, snpInfo, chrs,locusID, fixedRegSize, priorPi, estPi, varGenotypic, varResidual, chainLength, burnIn, outputFreq, onScreen)
    ...
    for iter in 1:chainLength
        ...
        for r in 1:nRegions
            ...
            logD0 = -(0.5)*(regionSize*log(det(varD0)))+sum(inv(varD0)*dot(rhsReg,rhsReg)) + logPiD
            logD1 = -(0.5)*(regionSize*log(det(varD1)))+sum(inv(varD1)*dot(rhsReg,rhsReg))  + logPiDComp
